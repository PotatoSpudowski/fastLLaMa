<template>
    <sp-dialog v-if="show" class="w-[25rem] bg-zinc-900" no-divider dismissable @close="emits('update:show', false)">
        <div slot="heading">
            ...
            <sp-search :value="currentPath" class="w-[15rem]"></sp-search>
        </div>

        <div class="max-h-[10rem] overflow-y-auto">
            <sp-menu class="w-full" selects="single" @change="onItemSelect">
                <sp-menu-item v-for="file in files" :key="file.kind + file.path" :value="`${file.path};${file.kind}`"
                    @dblclick="openDir(file.path)">
                    <ion-icon slot="icon" :name="file.kind === 'directory' ? 'folder' : 'document'" />
                    <div class="flex items-center gap-2 w-full justify-between">
                        <span>{{ file.name }}</span>
                        <ion-icon aria-hidden="true" name="chevron-forward-outline"
                            v-if="file.kind === 'directory'"></ion-icon>
                    </div>
                </sp-menu-item>
            </sp-menu>
        </div>
        ...
        <div class="flex flex-col">
            <sp-button-group class="min-w-fit mt-2">
                <sp-button variant="secondary" treatment="outline" @click="goBack">Go Back</sp-button>
...
function onItemSelect(e: Event) {
    ...
    selectedFile.value = files.value.find(file => file.path === path && file.kind === type);
}

// ---------------------------- File Manager ------------------------

const { files, currentPath } = storeToRefs(useAppStore());
const { openDir, goBack } = useAppStore();
